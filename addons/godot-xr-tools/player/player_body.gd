...
enum GroundControl { ON_GROUND, NEAR_GROUND, ALWAYS }  ## Apply ground control only when on ground  ## Apply ground control when near ground  ## Apply ground control always
...
@export var player_radius: float = 0.2:
...
func _estimate_body_forward_dir() -> Vector3:
	...
	if (
		...
		var tangent = (
			right_hand_node.global_transform.origin - left_hand_node.global_transform.origin
		)
...
func _update_body_under_camera(delta: float):
	...
	if _player_height_override_enabled:
		...
		_player_height_override_lerp = min(
			_player_height_override_lerp + player_height_rate * delta, 1.0
		)
	else:
		# Slew towards height being controlled by player
		_player_height_override_lerp = max(
			_player_height_override_lerp - player_height_rate * delta, 0.0
		)

	# Blend the player height between the player and software-override
	player_height = lerp(
		player_height, _player_height_override_current, _player_height_override_lerp
	)
	...
	if player_height > current_height:
		...
		var target_height: float = min(current_height + player_height_rate * delta, player_height)
		...
		# ceiling.
		var reduced_height: float = max(current_height - 0.1, player_radius)
		...
		player_height = max(reduced_height + grow * safe, current_height)
...
func _apply_velocity_and_control(delta: float):
	...
	if _can_apply_ground_control() and ground_control_velocity.length() >= 0.1:
		...
		control_velocity = (
			(dir_forward * -ground_control_velocity.y + dir_right * ground_control_velocity.x)
...
static func find_instance(node: Node) -> XRToolsPlayerBody:
	return (
		XRTools.find_xr_child(XRHelpers.get_xr_origin(node), "*", "XRToolsPlayerBody")
